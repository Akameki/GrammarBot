...
module.exports = {
  ...
  async execute(message, args) {
    ...
    function sendProperUsage() {
      return message.channel
        .send(`Usage: ${message.client.prefix}${usage}`)
        .catch(console.error);
    }
    ...
    if (args.length == 2) {
      ...
      if (
        (args[1].toLowerCase() === "all" || args[1].toLowerCase() === "new") &&
    ...
    if (message.client.judging)
      return message.channel.send(
        "please wait for the previous judgement to be completed or stopped"
      );

    if (args[1] <= 0) {
      return message.channel
        .send("limit must be positive you dunce")
        .catch(console.error);
    } else if (args[1] > 30) {
    ...
    message.client.data[channelId][targetId] = message.client.data[channelId][
      targetId
    ...
    if (newMode) {
      ...
      var targetDataOldestMessage = await message.channel.messages.fetch(
        targetData.oldestMessageId
      );
    }
    ...
    embedMessage = await message.channel
    ...
    let correct = 0;
    targetData.oldestMessageId =
      targetData.oldestMessageId ?? filteredMessages[0].id;
    let oldestMessage = await message.channel.messages.fetch(
      targetData.oldestMessageId
    );
    ...
    reactionCollector.on("collect", async (reaction, user) => {
      try {
        ...
        if (currentMessage.createdAt < oldestMessage.createdAt) {
          // will always be true after first time it's true
    ...
    reactionCollector.on("end", (collected, reason) => {
      ...
      const sessionPercentage = (
        (100 * correct) /
        (correct + incorrect)
      ).toFixed(1); //Math.round(10 * 100 * correct / (correct + incorrect)) / 10
      const totalPercentage = (
        (100 * targetData.correct) /
        (targetData.correct + targetData.incorrect)
      ).toFixed(1); //Math.round(10 * 100 * targetData.correct / (targetData.correct + targetData.incorrect)) / 10
      let heading, color;
      switch (reason) {
        case "complete":
      ...
      const resultsEmbed = new Discord.MessageEmbed()
        .setColor(color)
        .setAuthor(
          `${heading} ${targetMember.nickname}`,
          targetMember.user.avatarURL()
        )
        .addField(
          "This session",
          `${correct} ✅ ${incorrect} ❌ - ${sessionPercentage}%`,
          true
        )
        .addField(
          "Total in channel",
          `${targetData.correct} ✅ ${targetData.incorrect} ❌ - ${totalPercentage}%`,
          true
        )
        .setFooter(
          `Judge ${message.member.nickname}`,
...
async function createEmbeds(messages, targetMember, targetData) {
  ...
  for (let message of messages) {
    const embed = new Discord.MessageEmbed()
      .setColor("#aaaaaa")
      .setAuthor(
        `The Case of ${targetMember.nickname}`,
        targetMember.user.avatarURL()
      )
      .setDescription(message.cleanContent)
      .addField("Time created", message.createdAt.toDateString(), true)
      .setFooter(
        `Judge ${message.member.nickname}, exhibit ${page++} of ${
          messages.length
        }`,
        message.author.avatarURL()
      );
    ...
    if (targetData[message.id]) {
      ...
      const judgeMember = await message.guild.members
        .fetch(judge)
        .catch(console.error);
      switch (verdict) {
        case "correct":
